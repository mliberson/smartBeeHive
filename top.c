...
char *splash_screen[SCREEN_WIDTH] =  {"                    ",
                                      ...
                                      "    TO THE HIVE     ",
                                      "                    "};
char *options_menu[SCREEN_WIDTH] =   {"SEE CURRENT READINGS",
                                      "SET A PARAMETER     ",};
...
unsigned char state;
unsigned char options_menu_ind = 0;
...
int main()
{
    ...
    /* Setup home screen */
    state = HOME;
    lcd_screen(home_screen, HOME_SCRN_HT);
    lcd_moveto(2,0);
    lcd_stringout(options_menu[options_menu_ind]);
    lcd_moveto(3,0);
    lcd_stringout(options_menu[options_menu_ind+1]);
...
void update_options_menu()
{
    ...
    int i;
    for(i = HOME_SCRN_HT; i < SCREEN_HEIGHT; i++)
    {
        lcd_moveto(i, 0);
        lcd_stringout(options_menu[options_menu_ind+(i-HOME_SCRN_HT)]);
    }
}
